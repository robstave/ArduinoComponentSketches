...
//                  GND 4|    |5  PB0 (pin0) classic = high (optional)
//
...
// If mode is classic, then there is no tuning, we just use these numbers.
// Note, these are not really frequencies, just numbers that seem to work.
// Classic freq is 540 and 800 Hz
...
void loop()
{
  ...
  while (true)
  {
    ...
    // Current code just looks at pot.  if turned all the way down, it assumes
    // Classic, but you can put a bypass on pb0 too.
...
ISR(TIMER0_COMPA_vect)
{
  ...
  // you could do this a few ways.  But we are really just dealing with
  // squares...so this made sense at the time.
  // for example
  //  byte x1 = (Acc1 >> 8) & 0x80; is pretty much going to be 0 or 128
  //  if you shifted that by one ( div/2) and summed, then you would get
  ...
  // so unwinding the math can be a good thing.
